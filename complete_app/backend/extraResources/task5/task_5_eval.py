...
def getHandle(object):
    returnCode,handle=sim.simxGetObjectHandle(client_id,object,sim.simx_opmode_blocking)
...
returnCode,diff_drive_bot=getHandle('./Diff_Drive_Bot')
...
returnCode,last_handle=getHandle('./last_handle')
...
#     print(i)
#REMOVE FRUITS
returnCode,tree=getHandle('./tree')
returnCode,fs0=getHandle('./tree/ForceSensor[0]')
returnCode,tree_end=getHandle('./tree/tree_end')
...
for i in range(1,5):
    ...
    for j in range(3):
        for k in range(1,4):
            ref=ref1+'_L'+str(k)
            returnCode,ref_dummy=getHandle(ref)
            if j==0:
            ...
            if random_no == 1 and r<3:
                r+=1
                fruit_holder = cwd+dev+"/backend/extraResources/task5/models/sheet_"+str(i)+"_ripe.ttm"
                ...
                u+=1
                fruit_holder = cwd+dev+"/backend/extraResources/task5/models/sheet_"+str(i)+"_unripe.ttm"
                returnCode,fruit_holder_handle=sim.simxLoadModel(client_id,fruit_holder,0,sim.simx_opmode_blocking)
                returnCode=sim.simxSetObjectPosition(client_id,fruit_holder_handle,-1,fruit_pos,sim.simx_opmode_oneshot)
                returnCode=sim.simxSetObjectParent(client_id,fruit_holder_handle,tree,True,sim.simx_opmode_oneshot)
...
tree_ends = cwd+dev+"/backend/extraResources/task5/models/tree_end.ttm"
...
last_han = cwd+dev+"/backend/extraResources/task5/models/last_handle.ttm"
...
score = ((total_ripe_fruits-len(untouched)-len(displaced))*100/total_ripe_fruits)*0.7+((total_unripe_fruits-len(unripe_displaced))*100/total_unripe_fruits)*0.3
print('pluck_score(out of 100)=',score)
score = score*0.6+line_following_score*0.4
if len(deposited)==0:
    score = 0
#Preparing data to send to servers
print('score=',score)
task_data_dict = {}
